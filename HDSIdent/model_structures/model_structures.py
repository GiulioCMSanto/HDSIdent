...
class ModelStructure(object):
    ...
    def _qr_factorization(self, y, input_idx, X_cols, output_idx, y_cols, segment):
        ...
        Phi = self.Phi_dict['segment'+'_'+str(segment)] \
                           [output_idx_name] \
                           [input_idx_name]
        ...
        self.Phi_aug_dict['segment'+'_'+str(segment)] \
                         [output_idx_name] \
                         [input_idx_name] = np.zeros((len(segment_idx[y_shift:]),self.Nb + 1))
        ...
        self.Phi_aug_dict['segment'+'_'+str(segment)] \
                         [output_idx_name] \
                         [input_idx_name][:,-1] = \
            np.squeeze(y[segment_idx,output_idx][y_shift:])
        
        #QR-Factorization
        Q, R = LA.qr(self.Phi_aug_dict['segment'+'_'+str(segment)] \
                                      [output_idx_name] \
                                      [input_idx_name])
    ...
    def _compute_miso_ranks(self, X, y, regressor_mtrx, input_idx, X_cols, output_idx, y_cols, segment):
        """
        ...
        self.miso_ranks['segment'+'_'+str(segment)] \
                       [output_idx_name] \
                       [input_idx_name] = efr
    
    def _compute_miso_correlations(self, X, y, input_idx, X_cols, output_idx, y_cols, segment):
        """
        ...
        self.miso_correlations['segment'+'_'+str(segment)] \
                              [output_idx_name] \
                              [input_idx_name] = ncc